...
test("board includes paper and idle agents without ranking simulated returns, and deduplicates re-grants", async () => {
  ...
  try {
    await db.exec(`CREATE TABLE agents(smart_account TEXT, name TEXT, x_handle TEXT, x_verified INTEGER, epoch INTEGER, mode TEXT, created_at INTEGER, contributions_known INTEGER);
      ...
      INSERT INTO agents VALUES ('live','Live',NULL,0,1,'live',4,1),('paper','Paper',NULL,0,1,'paper',3,1),('idle','Idle',NULL,0,1,'idle',2,1),('old','Old grant',NULL,0,1,'live',1,1),('rh:hidden','Broker',NULL,0,1,'live',1,1);
      INSERT INTO equity VALUES ('live',1,110,1,1,'live'),('paper',1,10000,1,2,'paper');
      INSERT INTO flows VALUES ('live',1,'in',100),('paper',1,'in',100);
      INSERT INTO trades VALUES ('live',1,'landed',0),('paper',1,'paper',0);`);
    const identities = async () => [{tenant: '0x1' as const, slug: 'live-agent', accounts: ['live', 'old'] as `0x${string}`[], createdAt: 1, updatedAt: 1}];
