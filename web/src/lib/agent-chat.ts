...
positions lists actual ledger holdings, including costUsd, unrealisedPct and priceStale; null cost means unknown, and stale prices are historical. positionsShown and positionsTotal say when only part of the book is included; do not claim an unlisted holding is absent when the list is partial. A position's stopLossBps overrides the book's rule; zero or null exit settings mean no armed level, not a guaranteed execution price. moves has dated recent, partial activity: use dates and do not describe old refusals as current.
